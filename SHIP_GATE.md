...
- [x] `[cli|mcp|desktop]` Dangerous actions (kill, delete, restart) require explicit `--allow-*` flag (2026-02-27) — demo CLI only simulates errors, no destructive operations
- [x] `[cli|mcp|desktop]` File operations constrained to known directories (2026-02-27) — writes only to --json-out path
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[mcp]` SKIP: not an MCP server

## B. Error Handling

- [x] `[all]` Errors follow the Structured Error Shape: `code`, `message`, `hint`, `cause?`, `retryable?` (2026-02-27) — CliErrorV01 with code, title, what, why, fix (exceeds Tier 1)
- [x] `[cli]` Exit codes: 0 ok · 1 user error · 2 runtime error · 3 partial success (2026-02-27)
- [x] `[cli]` No raw stack traces without `--debug` (2026-02-27) — Click handles exceptions, structured output only
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[mcp]` SKIP: not an MCP server
...
- [ ] `[mcp]` SKIP: not an MCP server
- [ ] `[complex]` SKIP: reference demo, not complex enough for ops handbook
